...
describe('Blend Calculator', () => {
    let service: BlendCalculatorService;

    beforeEach(() => {
        service = new BlendCalculatorService();
    });
    ...
    it('should return optimal mix for 125 feet @ppO2 of 1.4', () => {
        //Act
    ...
    it('should return a valid EANx Blend starting with an empty tank, imperial', () => {
        //Arrange
        const mysrvice = new BlendCalculatorService();
        const request = new CalculationRequest();
        request.fillSpecs.pressure = 3000;
        request.fillSpecs.gasBlend.oxygen = 32.0;
        request.fillSpecs.gasBlend.nitrogen = 68.0;

        //Act
        const result = mysrvice.calculateFill(request);
